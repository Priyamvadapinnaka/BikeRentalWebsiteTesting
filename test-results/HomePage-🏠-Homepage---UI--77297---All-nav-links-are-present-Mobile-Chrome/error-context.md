...
- Name: HomePage.spec.js >> 🏠 Homepage - UI & Content >> TC03 - All nav links are present
- Location: tests\HomePage.spec.js:22:3
...
Locator:  locator('nav a').filter({ hasText: 'Home' })
Expected: visible
Received: hidden
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('nav a').filter({ hasText: 'Home' })
    8 × locator resolved to <a href="/" class="text-[11px] font-medium transition-colors hover:text-zinc-900 text-zinc-900">Home</a>
